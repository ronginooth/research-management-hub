...
async function fixCalendarIntegration() {
  ...
  try {
    ...
    dashboardContent = dashboardContent.replace(
      /const \[calendarError, setCalendarError\] = useState\(null\);/g,
      ''
    );

    // Add calendarError state declaration if not present
    if (!dashboardContent.includes('const [calendarError, setCalendarError] = useState(null);')) {
      dashboardContent = dashboardContent.replace(
  ...
  useEffect(() => {
    ...
    dashboardContent = dashboardContent.replace(
      /const fetchEvents = useCallback\(async \(\) => \{[\s\S]*?\}, \[isSignedIn, fetchEvents\]\);/,
      newFetchEvents
    );
...
export const listEvents = async () => {
  ...
  return new Promise((resolve, reject) => {
    ...
    tokenClient.callback = async (response: { error?: string; access_token?: string }) => {
